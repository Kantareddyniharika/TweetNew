...
namespace TweetAPP.Controller
{
    ...
    public class TweetAppController : ControllerBase
    {
        ...
        /// <returns>response.</returns>
        [HttpPost]
        [Route("reply/{comment},{username},{tweet},{date}")]
        public async Task<IActionResult> PostComment(string comment, string username, string tweet, DateTime date)
        {
            try
            {
                var result = await this.tweetAppService.Comments(comment, username, tweet, date);
                return this.Ok(result);
            }
            catch (Exception ex)
            {
